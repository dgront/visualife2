...
impl Mindmap {
    ...
    pub fn place_node(&mut self, id: impl Into<ElementID>, label: &str, x: f32, y: f32) -> ElementID {
        let id = id.into();
    ...
    pub fn grow_node(&mut self, id: impl Into<ElementID>, label: &str, angle_deg: f32, parent_node_id: &ElementID) -> ElementID {
    ...
    pub fn create_elements(&self) -> Vec<SvgElement> {
        ...
        let mut elements: Vec<SvgElement> = vec![];
        for node in self.nodes.values() {
            elements.push(node.create_elements());
        }
